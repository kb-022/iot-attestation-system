...
fn main() {
    ...
    if let Err(err) = block_on(async {
        ...
        let  temperatures = [25.6,23.3,26.7,21.0,56.9];
        for n in temperatures.iter() {
            // Create a message and publish it
            println!("Publishing a message on the topic 'temperature'");
            let payload = n.to_string();
            let msg = mqtt::Message::new("temperature", payload, mqtt::QOS_1);
